...
import { Project, StudentWork, Material, Activity } from './types';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'projects' | 'works' | 'materials' | 'activities' | 'about'>('about');
  ...
  const handleTabChange = (tab: 'about' | 'projects' | 'works' | 'activities' | 'materials', isMobile = false) => {
    setActiveTab(tab);
    setShowAdminDashboard(false);
    setShowForm(false);
    if (isMobile) {
  ...
  const visibleProjects = projects.filter(p => (p.approved && !p.rejected) || (isAdmin && !p.rejected) || (user && p.authorId === user.uid && !p.rejected));
  const visibleWorks = works.filter(w => (w.approved && !w.rejected) || (isAdmin && !w.rejected) || (user && w.authorId === user.uid && !w.rejected));
  const visibleMaterials = materials.filter(m => (m.approved && !m.rejected) || (isAdmin && !m.rejected) || (user && m.authorId === user.uid && !m.rejected));
  const visibleActivities = activities.filter(a => (a.approved && !a.rejected) || (isAdmin && !a.rejected) || (user && a.authorId === user.uid && !a.rejected));
      ...
      <main className="flex-grow flex flex-col pt-[57px] md:pt-[69px]">
          ...
          <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center relative z-10">
            ...
            >
              ...
              <div className="relative w-full max-w-[340px] md:max-w-[420px] lg:max-w-[460px] aspect-square bg-white border-4 border-brand-red p-2.5 shadow-[10px_10px_0px_0px_rgba(188,33,34,1)] -rotate-1 hover:rotate-1 transition-all duration-500 group select-none md:mr-4 z-20">
                ...
                <img 
                  src="/src/assets/images/mural_brigada_ramona_parra_1781765930415.jpg" 
        ...
        <section id="content-form-section" className="max-w-7xl mx-auto w-full px-6 md:px-12 py-16 scroll-mt-[75px] md:scroll-mt-[90px]">
          ...
          <AnimatePresence mode="wait">
            {showAdminDashboard ? (
              <AdminDashboard key="admin" user={user} projects={projects} works={works} materials={materials} activities={activities} />
            ) : (
              <motion.div
                key={activeTab}
              ...
              >
                ...
                {activeTab === 'works' && (
                  <div className="space-y-12">
                    <FilterBar selected={selectedAsignatura} onSelect={setSelectedAsignatura} />
                    {visibleWorks.filter(w => !selectedAsignatura || w.workType === selectedAsignatura).length === 0 ? (
                      <EmptyState text="No hay trabajos aprobados en esta asignatura autónoma." />
                    ) : (
                      <RetroCarousel itemCount={visibleWorks.filter(w => !selectedAsignatura || w.workType === selectedAsignatura).length}>
                        {visibleWorks
                          .filter(w => !selectedAsignatura || w.workType === selectedAsignatura)
                          .map((work, i) => (
                            <div key={work.id} className="w-[85vw] sm:w-[360px] md:w-[380px] lg:w-[400px] shrink-0 snap-start">
                              <WorkCard work={work} user={user} dark={i % 2 === 1} />
                            </div>
                          ))
                        }
                      </RetroCarousel>
  ...
  return (
    ...
    >
      ...
      <div className="mb-6">
        ...
        <h4 className="text-3xl lg:text-4xl font-extrabold leading-[1] tracking-tight mb-6 uppercase text-brand-red font-sans">
          {work.title}
        </h4>
        ...
        <div className={cn("relative p-6 mb-6 border-l-4 border-brand-red", dark ? "bg-white/5" : "bg-brand-bg")}>
          ...
          <p className={cn("text-sm md:text-base leading-relaxed font-serif italic line-clamp-8 pt-4", dark ? "text-white/95" : "text-gray-800")}>
            {work.content}
          </p>
        ...
        {work.link && (
          <a 
            ...
            className={cn("inline-flex items-center text-[10px] font-bold uppercase border-b-2 gap-2 mb-4", dark ? "text-brand-red border-brand-red hover:text-white hover:border-white" : "text-brand-red border-brand-red hover:text-black hover:border-black")}
          >
...
function MaterialCard({ material, user, isSelected, onSelect }: MaterialCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="p-6 bg-brand-black text-white border-b-4 border-brand-black -mx-6 -mt-6 mb-6">
        <div className="text-[10px] font-mono font-extrabold tracking-[0.2em] text-brand-red mb-3 uppercase">RECURSO DOCENTE</div>
      ...
      <div className="flex-grow flex flex-col justify-between">
        ...
        <div className="flex items-center gap-3 mb-6 p-3 bg-gray-50 border border-brand-border">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs font-black uppercase text-brand-black">{material.teacherName}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 mt-auto">
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-brand-border">
          {material.fileUrl ? (
            <a href={material.fileUrl} target="_blank" rel="noopener noreferrer" className="flex-grow">
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-grow gap-4 pt-6">
        {activity.imageUrl ? (
          <div className="aspect-video w-full overflow-hidden bg-brand-bg border-b-4 border-brand-black relative">
            <img 
              src={activity.imageUrl} 
              alt={activity.title}
              className="object-cover w-full h-full filter hover:filter-none transition-all duration-500"
              referrerPolicy="no-referrer"
            />
          </div>
        ) : (
        ...
        <div className="flex-grow space-y-4 px-2">
          <div className="space-y-2">
            <span className="text-[10px] font-mono tracking-[0.2em] font-extrabold text-brand-red uppercase block">ACTIVIDAD COLECTIVA</span>
...
function AdminDashboard({ projects, works, materials, activities, user }: AdminDashboardProps) {
  const confirm = useRetroConfirm();
  const [adminTab, setAdminTab] = useState<'projects' | 'works' | 'materials' | 'activities'>('works');
  ...
  const currentList = adminTab === 'projects' ? projects : adminTab === 'works' ? works : adminTab === 'materials' ? materials : activities;
  ...
  const authors = Array.from(new Set(currentList.map(item => {
    ...
    if (adminTab === 'materials') return item.teacherName || 'Docente';
  ...
  const handleBulkAction = async (action: 'approve' | 'reject' | 'delete') => {
    ...
    const table = adminTab === 'projects' ? 'projects' : adminTab === 'works' ? 'studentWorks' : adminTab === 'materials' ? 'materials' : 'activities';
...
function AboutSection() {
  ...
  const principles = [
    ...
    },
    {
      ...
      fullText: "Rechazamos firmemente la idea de que los sectores populares son meros consumidores pasivos de cultura ajena. Impulsamos metodologías pedagógicas críticas que sitúan a los estudiantes en el rol activo de creadores y productores intelectuales de conocimiento, literatura, arte, tecnología y soluciones comunitarias reales. Fomentamos que la autoría se construya desde la propia voz auténtica de nuestra comunidad.",
    ...
    <div className="max-w-5xl space-y-32">
      <section className="relative overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-16 text-base md:text-lg text-gray-700 leading-relaxed font-serif italic max-w-4xl">
          <p>
            Este espacio digital se concibe como una red de aprendizajes, conocimientos, resultados y demostraciones pedagógicas donde las y los estudiantes son los protagonistas al exponer sus propios trabajos.
          </p>
          <p>
      ...
      <section className="grid lg:grid-cols-3 gap-1 px-1 bg-brand-border">
        ...
        <AboutStat label="Voz" value="Identidad" desc="El estudiante como autor de su historia." />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {contentType === 'works' && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase text-gray-400">Curso o Año</label>
                  <input name="year" required placeholder="Ej: 3ro Medio Vespertino" className="bg-white border border-brand-border px-3 py-2 text-sm outline-none focus:border-brand-red" />
                </div>
              </div>
              <FormInput name="link" label="Enlace / Link (Opcional)" placeholder="https://..." />
          ...
          {contentType === 'materials' && (
            <div className="space-y-4">
              ...
              <FormInput name="fileUrl" label="Enlace al Archivo" placeholder="Google Drive, Dropbox, etc." />
          ...
          <div className="pt-4 border-t border-gray-100">
            <PolishedButton type="submit" className="w-full bg-brand-red text-white">
              {loading ? 'Procesando...' : isAdmin ? 'Publicar Ahora' : 'Enviar para Revisión'}
            </PolishedButton>
          </div>
        </form>
      </PolishedCard>
...
function FormInput({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input 
        className="bg-white border border-brand-border px-3 py-2 text-sm outline-none focus:border-brand-red transition-colors" 
        {...props} 
      />
    </div>
  );
        ...
        {activeItems.map((item, index) => (
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-600 font-serif italic line-clamp-4 leading-relaxed">
                {item.description}
              </p>
            </div>
            ...
            <div className="pt-4 border-t border-gray-100 flex flex-col gap-2 mt-6">
              ...
              {item.teacherName && (
                <div className="flex justify-between items-center text-[10px]">
                  <span className="text-gray-400 uppercase font-mono">Docente:</span>
