...
export function PolishedButton({ children, className, onClick, variant = 'primary', type = 'button', disabled = false }: PolishedButtonProps) {
...
interface RetroCarouselProps {
  ...
  itemCount: number;
  autoplay?: boolean;
  autoplayInterval?: number; // ms between advances, default 3500
}

export function RetroCarousel({ children, itemCount, autoplay = false, autoplayInterval = 3500 }: RetroCarouselProps) {
  ...
  // Autoplay: advance one card-width automatically
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      const cardWidth = Math.min(clientWidth * 0.85, 420);
      const isAtEnd = scrollLeft + clientWidth >= scrollWidth - 10;
      el.scrollBy({
        left: isAtEnd ? -scrollWidth : cardWidth,
        behavior: 'smooth',
      });
    }, autoplayInterval);
